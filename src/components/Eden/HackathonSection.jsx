                    ...
                    <div className={'flex justify-center'}>
                        <div className={'w-10/12 md:w-8/12 lg:w-5/12 flex gap-y-4 md:gap-y-6 flex-wrap'}>
                            <motion.h2
                                ...
                                viewport={{once: true}}
                                className={'font-spartan font-[500] text-[40px] md:text-[52px] xl:text-[80px] text-center tracking-[-0.05em] leading-[0.9em]'}>{data.hackathon.title}</motion.h2>
                            <motion.p
                                initial={{opacity: 0, y: 20}}
                                whileInView={{opacity: 1, y: 0}}
                ...
                <div className={'w-full pt-10 md:pt-20'}>
                    <div className={'w-full aspect-square md:aspect-video lg:aspect-[1280/512] relative'}>
                        ...
                        <motion.button
                            ...
                            viewport={{once: true}}
                            className={'absolute left-1/2 top-1/2 bg-white flex py-5 px-10 gap-x-3 items-center uppercase font-aeonik text-base font-[500] leading-[1.23em]'}>
                ...
                <div className={'w-full pt-10 lg:pt-20'}>
                    <div className={'grid grid-cols-1 md:grid-cols-3 gap-3 xl:gap-4'}>
                        {data.hackathon.elements.map((item, index) => (
                            <div key={index} className={`gap-y-[60px] flex flex-wrap text-white p-8 lg:p-12 bg-cover bg-no-repeat`} style={{backgroundImage: `url(${item.background})`}}>
                                <motion.div
                                    initial={{opacity: 0, y: -20}}
                                    whileInView={{opacity: 1, y: 0}}
                                    transition={{duration: 0.8, ease: "easeOut", delay: index * 0.2}}
                                    viewport={{once: true}}
                                    className={'self-start w-full text-[32px] lg:text-[40px] font-[600] font-spartan leading-[0.9]'}>{item.title}</motion.div>
                                <motion.div
                                    initial={{opacity: 0, y: 20}}
                                    whileInView={{opacity: 1, y: 0}}
                                    transition={{duration: 0.8, ease: "easeOut", delay: 0.2 + index * 0.2}}
                                    viewport={{once: true}}
                                    className={'self-end w-full font-aeonik text-sm lg:text-base font-[500] leading-[1.5em]'}>{item.text}</motion.div>
                            </div>
                        ))}
                    </div>
                </div>
