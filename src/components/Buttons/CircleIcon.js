const CircleIcon = ({
    ...
    return (
        <div className={`group h-12 w-12 rounded-full border border-white ${className} relative overflow-hidden`}>
            <div className={`absolute top-0 left-0 h-full w-full ${rotateClass(direction)}`}>
                <div className="absolute top-0 left-0 h-full w-full group-hover:-top-full transition-all duration-300">
                    ...
                    <div className={`top-full left-0 absolute h-full w-full flex justify-center items-center ${rotateClass(hoverDirection)}`}>
