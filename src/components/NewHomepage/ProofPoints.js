 ...
 * column reflow). Numerals use Slussen Expanded semibold.
 */
const ProofPoints = () => {
	return (
					...
					{stats.map((stat, index) => (
						<div key={stat.number} className="contents">
							<div className="flex flex-col items-center gap-3 text-center min-[900px]:flex-1">
								<span className="flex items-center justify-center min-h-[56px] font-slussenExpanded font-semibold text-[32px] min-[900px]:text-[56px] leading-none tracking-[-0.02em] text-[#FDFCFF]">
									{stat.number}
								</span>
