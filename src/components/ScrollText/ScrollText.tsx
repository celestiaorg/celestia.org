...
const ScrollText = ({ children, lightMode = false, id, ...props }) => {
	...
	return (
		<section
			id={id}
			className={`py-20 px-4 lg:pt-36 ${lightMode ? "lg:pb-16" : "lg:pb-36"} overflow-hidden ${
			...
			{children.map((child, index) => {
				return (
					<AnimateScrollText index={index} key={index}>
						<ScaledText key={index} className={`text-center font-youth ${gradientText ? `gradient-text-${gradientType}` : ""}`}>
							{child}
						</ScaledText>
					</AnimateScrollText>
				);
				...
				return (
					<AnimateScrollText index={index} key={index}>
						<ScaledText
							...
							className={`text-center font-youth ${gradientText ? `gradient-text-${gradientType}` : ""}`}
						>
							{child}
						</ScaledText>
					</AnimateScrollText>
