...
export default async function WhatIsCelestia() {
	...
	return (
		<>
			...
			<ScrollNavigation sectionDetails={Object.fromEntries(Object.entries(tableOfContents).slice(-3))}>
						...
						<Col width={50}>
							<HeadingWithSuperscript>
								...
								<HeadingWithSuperscript.Superscript>
									<Body size='sm' className={"text-right lg:text-left"}></Body>
								</HeadingWithSuperscript.Superscript>
							</HeadingWithSuperscript>
						</Col>
					...
					<Row className={"gap-6"} align>
						<Col width={50}>
							<Image
								...
								height={800}
								alt='FPO Image'
								className='block w-full h-auto mx-auto mb-8'
								style={{ maxWidth: 800 }}
							/>
						</Col>
						<Col width={50}>
							<Body size={"md"} className={"mb-4"}>
								Data availability answers the question, has the data for this blockchain been published? It is critical to the
								security of any blockchain because it ensures that anyone can inspect the ledger of transactions and verify it.
							</Body>
							<Body size={"md"} className={"mb-4"}>
								Users of a monolithic blockchain usually download all the data to check that it is available.
							</Body>
							<Body size={"md"}>
								As blocks get bigger, it becomes impractical for normal users to download all the data meaning that they can’t verify
								the chain. Modular chains solve this problem by making it possible for users to verify very large blocks using a
				...
				<ScrollSection index={1} id={"data-availability-sampling"} canCopyLink>
					<Row className={`lg:gap-6 mb-8 lg:mb-10`}>
						...
						<Col width={50}>
							<Body size={"md"} className={"mb-4"}>
								Data availability sampling (DAS) is the new primitive that enables Celestia light nodes to verify DA efficiently.
								Instead of downloading all data, light nodes only download a tiny portion of each block.
							</Body>
							<Body size={"md"} className={"mb-4"}>
								Importantly, DAS allows Celestia to scale with the number of users (light nodes). So, as the light node network grows
								over time, Celestia can scale to the data throughput needed for millions of rollups without compromising on security
								for end users.
							</Body>
						</Col>
					</Row>
					...
					<Row className={`lg:gap-6 mb-8 lg:mb-10`}>
						...
						<Col width={50}>
							<Body size={"md"} className={"mb-4"}>
								...
								scalability, flexibility, and interoperability, enabling developers to build blockchain applications for mass adoption
							</Body>
						</Col>
					</Row>
					<Row className={"gap-6"}>
			...
			<section id={"start-using-celestia"} className='relative z-10 text-white bg-black'>
						...
						<Col width={100}>
							<div className='grid w-full grid-cols-1 gap-4 mb-4 md:grid-cols-2'>
								...
								<VerticalTitleCard
									...
									description={"Explore Celestia’s ecosystem of apps and services, run a light node, or learn about Tia."}
									dark={true}
